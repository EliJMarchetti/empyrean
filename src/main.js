...
async function confirmDeveloperChanges() {
  ...
  const issueUrl = await buildSystemContentIssueUrl(json);

  window.location.href = issueUrl;
}

async function buildSystemContentIssueUrl(json) {
  const encodedPayload = await encodeSystemContentIssuePayload(json);
  const body = [
    "This issue was generated by the Empyrean Developer Console.",
    "",
    "Submit this prefilled issue to apply the system-content update. A GitHub Action will validate and commit it if you own this repository.",
    "",
    "<!-- EMPYREAN_SYSTEM_CONTENT_UPDATE",
    `encoding: ${encodedPayload.encoding}`,
    "payload:",
    encodedPayload.payload,
    "END_EMPYREAN_SYSTEM_CONTENT_UPDATE -->",
  ].join("\n");
  const url = new URL(SYSTEM_CONTENT_GITHUB_ISSUE_URL);
  url.searchParams.set("title", SYSTEM_CONTENT_ISSUE_TITLE);
  url.searchParams.set("body", body);
  return url.toString();
}

async function encodeSystemContentIssuePayload(json) {
  if (typeof CompressionStream === "function" && typeof Blob === "function" && typeof Response === "function") {
    try {
      ...
      // Fall through to plain base64url when compression is unavailable.
    }
  }

  return {
    encoding: "base64url",
    payload: bytesToBase64Url(new TextEncoder().encode(json)),
  };
}
